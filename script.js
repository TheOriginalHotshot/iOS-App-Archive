            ...
            tab.addEventListener('click', function() {
                ...
                if (tabName === 'featured') {
                    ...
                    if (searchTerm && searchTerm.trim().length > 0) {
                        ...
                        // No search term, show category browsing
                        searchResults.classList.remove('active');
...
// ==================== iOS 6 CLEAN FINAL FIX ==================== //

// Category icons mapping
const categoryIcons = {
    'Games': { icon: 'fa-gamepad', class: 'games', color: '#ff453a' },
    'Entertainment': { icon: 'fa-film', class: 'entertainment', color: '#ff9f0a' },
    'Food & Drink': { icon: 'fa-utensils', class: 'food', color: '#32d74b' },
    'News': { icon: 'fa-newspaper', class: 'news', color: '#64d2ff' },
    'Utilities': { icon: 'fa-wrench', class: 'utilities', color: '#bf5af2' },
    'Social Networking': { icon: 'fa-users', class: 'social', color: '#5e5ce6' },
    'Productivity': { icon: 'fa-briefcase', class: 'productivity', color: '#30d158' },
    'Education': { icon: 'fa-graduation-cap', class: 'education', color: '#ffd60a' },
    'Music': { icon: 'fa-music', class: 'entertainment', color: '#ff9f0a' },
    'Photo & Video': { icon: 'fa-camera', class: 'entertainment', color: '#ff9f0a' },
    'Shopping': { icon: 'fa-shopping-cart', class: 'utilities', color: '#bf5af2' },
    'Travel': { icon: 'fa-plane', class: 'entertainment', color: '#ff9f0a' },
    'Sports': { icon: 'fa-football-ball', class: 'games', color: '#ff453a' },
    'Health & Fitness': { icon: 'fa-heartbeat', class: 'utilities', color: '#bf5af2' },
    'Finance': { icon: 'fa-dollar-sign', class: 'utilities', color: '#bf5af2' }
};

// Render Categories - Edge to Edge List
function renderCategoriesIOS6() {
    const categoriesContent = document.getElementById('categoriesContent');
    if (!categoriesContent || !appsLoaded) return;
    ...
    apps.forEach(app => {
        if (Array.isArray(app.categories)) {
            app.categories.forEach(cat => categoriesSet.add(cat));
        }
    });
    ...
    categories.forEach(category => {
        const iconData = categoryIcons[category] || { icon: 'fa-th-large', color: '#666666' };
        
        const item = document.createElement('div');
        item.className = 'category-item-ios6';
        item.innerHTML = `
            <div class="category-icon-left" style="color: ${iconData.color};">
                <i class="fas ${iconData.icon}"></i>
            </div>
            <div class="category-name-center">${escapeHtml(category)}</div>
            <button class="category-button-right">View Apps</button>
        `;
...
// Render Genius - Rectangular List
function renderGeniusIOS6() {
    const geniusContent = document.getElementById('geniusContent');
    if (!geniusContent) return;
    ...
    header.className = 'genius-header';
    header.innerHTML = `
        <h2><i class="fas fa-atom"></i> Recommended Apps</h2>
        <p>${viewedApps.length > 0 ? 'Based on apps you\'ve viewed' : 'Curated selections for you'}</p>
    `;
    container.appendChild(header);
    ...
    if (recommendations.length === 0) {
        container.innerHTML += '<p style="text-align: center; padding: 40px; color: #666;">Browse some apps to get recommendations!</p>';
    } else {
        const listContainer = document.createElement('div');
        listContainer.className = 'genius-list-container';
        
        recommendations.slice(0, 8).forEach(app => {
            const item = document.createElement('div');
            item.className = 'genius-item-ios6';
            item.innerHTML = `
                <div class="genius-icon-60">
                    ${app.icon ? `<img src="${escapeHtml(app.icon)}" alt="${escapeHtml(app.title)}">` : 
                    '<i class="fas fa-mobile-alt" style="font-size: 30px; color: #888;"></i>'}
                </div>
                <div class="genius-info-center">
                    <div class="genius-name">${escapeHtml(app.title)}</div>
                    <div class="genius-developer">${escapeHtml(app.developer || 'Unknown')}</div>
                </div>
                <button class="category-button-right">View</button>
            `;
...
function openModalIOS6(appId) {
    ...
    // Black top bar
    const header = document.createElement('div');
    header.className = 'ios6-modal-black-header';
    header.innerHTML = `
        <button class="modal-close-left">Close</button>
        <div class="modal-title-center">Info</div>
    ...
    // App header
    content.innerHTML = `
        <div class="ios6-modal-app-header">
            ...
            <div class="ios6-modal-app-info">
                ...
                <div class="ios6-modal-app-developer">${escapeHtml(app.developer || 'Unknown')}</div>
    ...
    segControl.className = 'segment-control-metallic';
    segControl.innerHTML = `
        <button class="segment-button active" data-segment="details">Details</button>
        <button class="segment-button" data-segment="related">Related</button>
    `;
    content.appendChild(segControl);
    ...
    detailsSection.innerHTML = `
        <div class="ios6-section-title">Information</div>
        <div class="ios6-details-content" style="padding: 15px;">
            <div style="margin-bottom: 15px;">
                <div style="font-size: 13px; color: #666; margin-bottom: 5px;">Compatibility</div>
                <div style="font-size: 15px; color: #000;">${escapeHtml(app.compatibility || 'Not specified')}</div>
            </div>
            <div>
                <div style="font-size: 13px; color: #666; margin-bottom: 5px;">Description</div>
                <div style="font-size: 15px; color: #000; line-height: 1.4;">${escapeHtml(app.description || 'No description')}</div>
            </div>
        </div>
    `;
    ...
    const relatedHTML = relatedApps.map(rApp => `
        <div class="ios6-related-item">
            <div class="ios6-related-icon">
                ${rApp.icon ? `<img src="${escapeHtml(rApp.icon)}" alt="${escapeHtml(rApp.title)}">` : 
                '<i class="fas fa-mobile-alt"></i>'}
            </div>
            <div class="ios6-related-info">
                <div class="ios6-related-name">${escapeHtml(rApp.title)}</div>
                <div class="ios6-related-developer">${escapeHtml(rApp.developer || 'Unknown')}</div>
            </div>
            <button class="category-button-right" data-app-id="${rApp.id}">View</button>
        </div>
    `).join('');
    ...
    relatedSection.innerHTML = `
        ...
        <div class="ios6-related-list">
            ${relatedHTML || '<p style="padding: 20px; text-align: center; color: #888;">No related apps</p>'}
        </div>
    `;
    ...
    // Close handler
    header.querySelector('.modal-close-left').addEventListener('click', () => {
        overlay.remove();
        document.body.style.overflow = 'auto';
    });
    ...
    // Segment buttons
    segControl.querySelectorAll('.segment-button').forEach(btn => {
        btn.addEventListener('click', function() {
            segControl.querySelectorAll('.segment-button').forEach(b => b.classList.remove('active'));
            this.classList.add('active');
            
            if (this.getAttribute('data-segment') === 'details') {
    ...
    // Related app buttons
    relatedSection.querySelectorAll('button[data-app-id]').forEach(btn => {
        btn.addEventListener('click', function() {
            const relAppId = this.getAttribute('data-app-id');
            overlay.remove();
            document.body.style.overflow = 'auto';
            setTimeout(() => openModalIOS6(relAppId), 100);
        });
    });
}

// Override the global openModal
window.openModal = openModalIOS6;

// ALL 3 News Changelogs
function renderAllNews() {
    ...
    // v2.1
    const card21 = createNewsCardSimple(
        'fa-palette',
        'iOS App Archive v2.1',
        'February 15, 2026',
        'iOS 6 refined design with metallic buttons, edge-to-edge categories, rectangular lists, and fixed modals.',
        'VERSION 2.1',
        'v2.1'
    );
    container.appendChild(card21);
    
    // v2.0
    const card20 = createNewsCardSimple(
        'fa-rocket',
        'iOS App Archive v2.0',
        'February 15, 2026',
        'Enhanced search with category rows, 3D cards, faster carousel, and category icons.',
        'VERSION 2.0',
        'v2.0'
    );
    container.appendChild(card20);
    
    // v1.0
    const card10 = createNewsCardSimple(
        'fa-star',
        'iOS App Archive v1.0',
        'February 15, 2026',
        'Genius recommendations, loading animation, and enhanced glossy UI effects.',
        'VERSION 1.0',
        'v1.0'
    );
...
function createNewsCardSimple(icon, title, date, preview, badge, version) {
    ...
    card.addEventListener('click', () => showNewsModalSimple(version));
    return card;
}

function showNewsModalSimple(version) {
    let icon, title, body;
    
    if (version === 'v2.1') {
        icon = 'fa-palette';
        title = 'iOS App Archive v2.1';
        body = `
            <h3>🎨 iOS 6 Refined Design</h3>
            <p>Complete visual refinement with authentic iOS 6 styling!</p>
            <ul>
                <li><strong>Featured Tab:</strong> iOS 6 grey gradient cards with 3D depth, black ingrained text, metallic buttons</li>
                <li><strong>Categories Tab:</strong> Edge-to-edge rectangular list, icons on left, View Apps buttons on right</li>
                <li><strong>Genius Tab:</strong> Rectangular list style with top 8 recommendations</li>
                <li><strong>Modal:</strong> Black glossy top bar, segmented control with proper slit, all buttons metallic grey</li>
                <li><strong>Search:</strong> Bigger cards (320px), larger icons, bigger counter text, no duplicate instructions</li>
                <li><strong>News:</strong> All three changelogs (v1.0, v2.0, v2.1) present and working</li>
            </ul>
        `;
        ...
        body = `
            ...
            <ul>
                <li>Enhanced search with category rows</li>
                <li>Category detail pages with "See All"</li>
                <li>Colorful category icons</li>
                <li>Genius icon changed to atom ⚛️</li>
                <li>3D skeuomorphic cards throughout</li>
                <li>Faster carousel (0.5s transitions)</li>
            </ul>
        `;
    } else {
        icon = 'fa-star';
        title = 'iOS App Archive v1.0';
        body = `
            <h3>🎉 Initial Major Update</h3>
            <ul>
                <li>Genius recommendations based on viewing history</li>
                <li>Beautiful loading animation on page load</li>
                <li>News tab for updates and changelogs</li>
                <li>Enhanced glossy UI effects throughout</li>
                <li>Local storage for tracking app views</li>
            </ul>
        `;
    }
    ...
    modal.innerHTML = `
        <div class="news-modal-content">
            ...
            <div class="news-modal-header">
                <div class="news-modal-icon"><i class="fas ${icon}"></i></div>
                <div class="news-modal-title-section">
                    <h2>${title}</h2>
                    <div class="news-modal-date">February 15, 2026</div>
                </div>
            </div>
            <div class="news-modal-body">${body}</div>
        </div>
    `;
...
if (typeof appsPromise !== 'undefined') {
    appsPromise.then(() => {
        if (appsLoaded) {
            renderCategoriesIOS6();
            renderGeniusIOS6();
            renderAllNews();
            
            // Tab handlers
            const categoriesTab = document.querySelector('.tab[data-tab="categories"]');
            if (categoriesTab) {
            ...
            const geniusTab = document.querySelector('.tab[data-tab="genius"]');
            if (geniusTab) {
                geniusTab.addEventListener('click', () => {
                    setTimeout(() => renderGeniusIOS6(), 100);
                });
            }

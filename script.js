...
// ==================== iOS 6 REFINED FUNCTIONALITY - V2.1 FINAL ==================== //

// Render Categories Tab - Edge to Edge with Icons and Buttons
function renderCategoriesTabIOS6() {
    ...
    categories.forEach(category => {
        const iconData = categoryIcons[category] || { icon: 'fa-th-large', class: '' };
        
        const item = document.createElement('div');
        item.className = 'category-list-item';
        item.innerHTML = `
            <div class="category-icon ${iconData.class}">
                <i class="fas ${iconData.icon}"></i>
            </div>
            <button class="category-select-btn" data-category="${escapeHtml(category)}">
                ${escapeHtml(category)}
            </button>
            <button class="category-view-apps-btn" data-category="${escapeHtml(category)}">
                View Apps
            </button>
        `;
        ...
        // Add click handlers
        const selectBtn = item.querySelector('.category-select-btn');
        const viewBtn = item.querySelector('.category-view-apps-btn');
...
// Render Genius Tab - Rectangular List Style
function renderGeniusTabIOS6() {
    const geniusContent = document.getElementById('geniusContent');
    if (!geniusContent) return;
    ...
    if (recommendations.length === 0) {
        ...
        noRecs.textContent = 'No recommendations available yet. Browse some apps to get personalized suggestions!';
        noRecs.style.textAlign = 'center';
        noRecs.style.color = '#666';
        noRecs.style.padding = '40px 20px';
        container.appendChild(noRecs);
    } else {
        const listContainer = document.createElement('div');
        listContainer.className = 'genius-apps-grid';
        
        // Show top 8 recommendations
        const topRecs = recommendations.slice(0, 8);
        
        topRecs.forEach((app, index) => {
            const item = document.createElement('div');
            item.className = 'genius-list-item';
            item.style.animation = `fadeInUp 0.3s ease ${index * 0.05}s both`;
            item.innerHTML = `
                <div class="genius-list-icon">
                    ${app.icon ? `<img src="${escapeHtml(app.icon)}" alt="${escapeHtml(app.title)}" loading="lazy">` : 
                    '<i class="fas fa-mobile-alt" style="font-size: 30px; color: #888;"></i>'}
                </div>
                <div class="genius-list-info">
                    <div class="genius-list-name">${escapeHtml(app.title)}</div>
                    <div class="genius-list-developer">${escapeHtml(app.developer || 'Unknown')}</div>
                </div>
                <button class="genius-list-button" data-app-id="${app.id}">View</button>
            `;
            
            listContainer.appendChild(item);
            
            // Add click handler
            const viewBtn = item.querySelector('.genius-list-button');
            viewBtn.addEventListener('click', function() {
                const appId = this.getAttribute('data-app-id');
                openIOS6ModalRefined(appId);
            });
        });
...
// Refined iOS 6 Modal with Black Top Bar and Better Styling
function openIOS6ModalRefined(appId) {
    trackAppView(appId);
    ...
    header.innerHTML = `
        <button class="ios6-modal-close">Close</button>
        <div class="ios6-modal-title">Info</div>
        <button class="ios6-modal-share"><i class="fas fa-share-square"></i></button>
    `;
    ...
    detailsSection.innerHTML = `
        ...
        <div class="ios6-details-content">
            ...
            <div class="ios6-detail-row">
                ...
                <div class="ios6-detail-value">${escapeHtml(app.description || 'No description')}</div>
            </div>
        </div>
    ...
    relatedSection.innerHTML = `
        ...
        <div class="ios6-related-list">
            ${relatedHTML || '<p style="padding: 20px; text-align: center; color: #8e8e93;">No related apps</p>'}
        </div>
    `;
    ...
    relatedSection.querySelectorAll('.ios6-related-button').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            overlay.remove();
            document.body.style.overflow = 'auto';
            setTimeout(() => openIOS6ModalRefined(relatedAppId), 100);
        });
    });
}

// Override openModal
window.openModal = openIOS6ModalRefined;

// Render All 3 News Items (v1.0, v2.0, v2.1)
function renderNewsPageComplete() {
    const updatesContent = document.getElementById('updatesContent');
    if (!updatesContent) return;
    ...
    // v2.1 - iOS 6 Redesign
    const card21 = createNewsCard(
        'fa-palette',
        'iOS App Archive v2.1',
        'February 15, 2026',
        'Complete iOS 6 App Store redesign with metallic buttons, refined modals, rectangular lists, and authentic styling.',
        'VERSION 2.1',
        () => showNewsModalComplete('2.1')
    );
    container.appendChild(card21);
    
    // v2.0 - Enhanced Features
    const card20 = createNewsCard(
        'fa-rocket',
        'iOS App Archive v2.0',
        'February 15, 2026',
        'Major update with category rows, 3D skeuomorphic cards, faster carousel, and category icons.',
        'VERSION 2.0',
        () => showNewsModalComplete('2.0')
    );
    container.appendChild(card20);
    
    // v1.0 - Initial Release
    const card10 = createNewsCard(
        'fa-star',
        'iOS App Archive v1.0',
        'February 15, 2026',
        'First major update with Genius recommendations, loading animation, and enhanced glossy UI effects.',
        'VERSION 1.0',
        () => showNewsModalComplete('1.0')
    );
...
function showNewsModalComplete(version) {
    ...
    if (version === '2.1') {
        ...
        bodyHTML = `
            <h3>🎨 iOS 6 Refined Styling</h3>
            <p>Complete visual refinement matching authentic iOS 6 App Store design!</p>
            
            <h3>✨ Featured Tab Updates</h3>
            <ul>
                <li>iOS 6 grey gradient cards with 3D depth effect</li>
                <li>Black ingrained text on all cards</li>
                <li>Metallic grey "View Details" buttons with glossy finish</li>
                <li>Proper iOS 6 button styling with highlights and shadows</li>
            </ul>
            
            <h3>📑 Categories Tab Redesign</h3>
            <ul>
                <li>Edge-to-edge rectangular boxes (no padding)</li>
                <li>Category icons on the left</li>
                <li>Category name in center with ingrained text</li>
                <li>Metallic grey "View Apps" buttons on right</li>
                <li>Vertical list layout matching iOS 6</li>
            </ul>
            
            <h3>💡 Genius Tab - List Style</h3>
            <ul>
                <li>Rectangular list items (not grid cards)</li>
                <li>App icon (60x60) on left</li>
                <li>App name and developer in center</li>
                <li>Metallic grey "View" button on right</li>
                <li>Top 8 recommendations displayed</li>
            </ul>
            
            <h3>📱 Modal Improvements</h3>
            <ul>
                <li>Black glossy top bar with "Info" title</li>
                <li>"Close" button on left with white text</li>
                <li>Share button on right</li>
                <li>Metallic grey segmented control (Details/Related)</li>
                <li>Proper slit/arrow below active segment</li>
                <li>All text ingrained and black</li>
                <li>Metallic grey "View" buttons in Related section</li>
            </ul>
            
            <h3>🔍 Search Page Fixes</h3>
            <ul>
                <li>Removed duplicate search instructions from below search box</li>
                <li>Instructions only show in center with magnifying glass</li>
                <li>Search result cards are 30% bigger (320px wide)</li>
                <li>Larger app icons (100x100)</li>
                <li>Bigger, ingrained counter text at bottom</li>
                <li>Black ingrained text throughout</li>
            </ul>
            
            <h3>📰 News Tab - Complete History</h3>
            <ul>
                <li>v2.1 - iOS 6 refinements (this update)</li>
                <li>v2.0 - Category rows and 3D cards</li>
                <li>v1.0 - Genius and loading animation</li>
            </ul>
        `;
    } else if (version === '2.0') {
        icon = 'fa-rocket';
        ...
        bodyHTML = `
            ...
            <ul>
                ...
                <li>Enhanced glossy UI effects</li>
                <li>Local storage for tracking views</li>
            </ul>
        `;
    }
...
// Initialize everything when apps load
            ...
            if (categoriesTab) {
                categoriesTab.addEventListener('click', () => {
                    setTimeout(() => {
                        const categoryParam = getUrlParam('category');
                        if (!categoryParam) {
                            renderCategoriesTabIOS6();
                        }
                    }, 100);
                });
            }
            
            // Render genius with new style
            renderGeniusTabIOS6();
            
            // Render news with all 3 versions
            renderNewsPageComplete();
        }
    });
}

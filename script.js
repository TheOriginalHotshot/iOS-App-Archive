        ...
        function renderAppsForCategory(cat){
            ...
            // ── "See All" sub-header bar (full-width, matches main header) ──
            const subHdr=document.createElement('div');
            subHdr.className='cat-seeall-header';
            subHdr.innerHTML=`
                <button class="cat-seeall-back-btn">
                    <i class="fa-solid fa-chevron-left"></i><span>Categories</span>
                </button>
                <span class="cat-seeall-title">See All</span>
                <div style="width:90px;"></div>`;
            subHdr.querySelector('.cat-seeall-back-btn').addEventListener('click',()=>{ setUrlParam('category',''); renderCategoryList(); });
            cc.appendChild(subHdr);

            const filtered=apps.filter(a=>Array.isArray(a.categories)&&a.categories.includes(cat));
            const sorted=[...filtered].sort((a,b)=>a.title.localeCompare(b.title));
            if(!sorted.length){ const p=document.createElement('p'); p.textContent='No apps in this category.'; p.style.cssText='text-align:center;color:#999;padding:20px;'; cc.appendChild(p); return; }

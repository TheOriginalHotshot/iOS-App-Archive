        ...
        function createModal(app) {
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-tab-content" id="reviews-content">
                        <div class="reviews-list" id="reviews-list-${app.id}">
                            <p class="reviews-empty-msg">No reviews yet. Be the first!</p>
                        </div>
                    </div>
            ...
            modal.querySelector('.modal-get-btn').addEventListener('click',()=>openFlipDownload(app, modal));
        ...
        function renderReviewsList(appId) {
            ...
            if (!reviews.length) {
                list.innerHTML = '<p class="reviews-empty-msg">No reviews yet. Be the first!</p>';
                return;
            }
            list.innerHTML = '';
        ...
        // ── Review write modal — OS X sheet animation (drops from modal sub-header) ──
        function openReviewModal(appId) {
            const existing = document.getElementById('reviewWriteModal');
            if (existing) existing.remove();

            // Attach the sheet to the open modal overlay so it drops FROM the header
            const parentModal = document.getElementById(appId + 'Modal');
            const attachTo = parentModal || document.body;

            const sheet = document.createElement('div');
            sheet.id = 'reviewWriteModal';
            sheet.className = 'review-sheet-osx';
            sheet.innerHTML = `
            ...
            attachTo.appendChild(sheet);
            // Force reflow then add active to trigger CSS transition (sheet drops down)
            sheet.offsetHeight;
            requestAnimationFrame(() => sheet.classList.add('osx-open'));

            const textarea = sheet.querySelector('#reviewTextarea');
            const sendBtn  = sheet.querySelector('#reviewSendBtn');
            const wrap     = sheet.querySelector('#reviewTextareaWrap');
            const cancelBtn= sheet.querySelector('.review-write-cancel-btn');

            function closeReviewSheet() {
                sheet.classList.remove('osx-open');
                setTimeout(() => sheet.remove(), 420);
            }
...
<li><strong>Glossy Skeuomorphic UI</strong> — dark brushed-metal tab bar, iOS 6 aesthetic.</li>
        ...
        window.addEventListener('load',function(){
            ...
            if(!ls) return;
            // Wait a beat so users see the icon, then fall away
            setTimeout(()=>{
                ls.classList.add('fall-away');
        ...
        (function initPullToRefresh() {
            ...
            const scrollableSelectors = ['#geniusContent','#updatesContent','#categoriesContent'];

            function setupPTR(el) {
                if (!el || el._ptrInit) return;
                el._ptrInit = true;
                ...
                ind.innerHTML = '<div class="ptr-arrow"><i class="fas fa-arrow-down"></i></div>';
                el.insertBefore(ind, el.firstChild);

                function getScrollTop() { return el.scrollTop || 0; }
                ...
                el.addEventListener('touchmove', e => {
                    ...
                    if (diff > 0 && getScrollTop() <= 0) {
                        const pull = Math.min(diff * 0.45, PTR_THRESHOLD + 20);
                        ind.style.height = pull + 'px';
                        ind.style.opacity = Math.min(pull / PTR_THRESHOLD, 1);
                        ind.querySelector('.ptr-arrow').style.transform =
                            `rotate(${Math.min((pull/PTR_THRESHOLD)*180, 180)}deg) scale(${Math.min(pull/PTR_THRESHOLD, 1)})`;
                    }
                }, { passive: true });
                ...
                el.addEventListener('touchend', () => {
                    ...
                    if (diff >= PTR_THRESHOLD && getScrollTop() <= 0) {
                        triggerRefresh(el, ind);
                    } else {
                        resetIndicator(ind);
                    }
                });

                // Desktop: wheel past top
                el.addEventListener('wheel', e => {
                    if (el.scrollTop <= 0 && e.deltaY < -30 && !refreshing) {
                        triggerRefresh(el, ind);
                    }
                }, { passive: true });
            }

            function triggerRefresh(el, ind) {
                const refreshing = true;
                ind.classList.add('ptr-refreshing');
                ind.style.height = PTR_THRESHOLD + 'px';
                ind.querySelector('.ptr-arrow').innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
                setTimeout(() => {
                    // Re-render the active page
                    if (el.id === 'geniusContent' && appsLoaded) renderGeniusPage();
                    else if (el.id === 'updatesContent') renderNewsPage();
                    else if (el.id === 'categoriesContent' && appsLoaded) renderCategoryList();
                    resetIndicator(ind);
                }, 900);
            }

            function resetIndicator(ind) {
                ind.style.transition = 'height 0.35s cubic-bezier(0.23,1,0.32,1), opacity 0.35s ease';
                ind.style.height = '0px';
                ind.style.opacity = '0';
                ind.classList.remove('ptr-refreshing');
                setTimeout(() => {
                    ind.style.transition = '';
                    if (ind.querySelector('.ptr-arrow'))
                        ind.querySelector('.ptr-arrow').innerHTML = '<i class="fas fa-arrow-down"></i>';
                }, 380);
            }

            // Attach PTR after each render (pages get re-created)
            const origGenius = window.renderGeniusPage;
            const origNews   = window.renderNewsPage;
            function patchPTR() {
            ...
            // Poll — safe since renders are infrequent
            setInterval(patchPTR, 800);
        })();

        ...
        function renderSearchResults(filteredApps = []) {
            ...
            sortedApps.forEach((app, index) => {
                ...
                appCard.innerHTML = `
                    <div>
                        ...
                        <div class="large-card-developer">${app.developer}</div>
                    </div>
                    <button class="large-card-button" data-app-id="${app.id}">View Details</button>
                `;
            ...
            scroller.addEventListener('scroll', function() {
                ...
                const cardWidth = scroller.querySelector('.app-card-large').offsetWidth;
                const currentIndex = Math.round(scrollLeft / cardWidth);
                counter.textContent = `${currentIndex + 1} of ${totalApps} ${totalApps === 1 ? 'app' : 'apps'}`;
            });

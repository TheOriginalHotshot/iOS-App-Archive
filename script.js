...
<li><strong>Glossy Skeuomorphic UI</strong> — dark brushed-metal tab bar, iOS 6 aesthetic.</li>
        ...
        // ── INJECT FAVOURITE BUTTON into createModal ────────────────────────
        // Patch the modal "get" button area to include a heart
        // We hook into openModal after modal is created
        const _origOpenModal = openModal;
        function openModal(appId) {
            _origOpenModal(appId);
            // After a frame, inject the fav button if not already there
            requestAnimationFrame(()=>{
                const modal = document.getElementById(appId+'Modal');
                if (!modal || modal._favInjected) return;
                modal._favInjected = true;
                const header = modal.querySelector('.modal-app-header');
                if (!header) return;
                const favBtn = document.createElement('button');
                favBtn.className = 'modal-fav-btn';
                favBtn.innerHTML = `<i class="${isFavourite(appId)?'fas':'far'} fa-heart"></i>`;
                favBtn.title = 'Favourite';
                favBtn.addEventListener('click',()=>{
                    const added = toggleFavourite(appId);
                    favBtn.innerHTML = `<i class="${added?'fas':'far'} fa-heart"></i>`;
                    favBtn.classList.toggle('fav-active', added);
                    // Bounce animation
                    favBtn.classList.remove('fav-bounce');
                    favBtn.offsetWidth;
                    favBtn.classList.add('fav-bounce');
                });
                if (isFavourite(appId)) favBtn.classList.add('fav-active');
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Favourites heart in header
            const heartBtn = document.createElement('button');
            heartBtn.className = 'header-favs-btn';
            heartBtn.innerHTML = '<i class="fas fa-heart"></i>';
            heartBtn.title = 'Favourites';
            heartBtn.addEventListener('click', renderFavouritesPage);
            if (hc) hc.appendChild(heartBtn);

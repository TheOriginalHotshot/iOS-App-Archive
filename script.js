        ...
        function openReviewModal(appId) {
            ...
            overlay.innerHTML = `
                <div class="review-write-sheet">
                    ...
                    <div class="review-write-body">
                        ...
                        <button class="review-send-btn action-btn" id="reviewSendBtn">Send</button>
                    </div>
                </div>`;
            ...
            sendBtn.addEventListener('click', () => {
                ...
                if (!text) { textarea.focus(); textarea.classList.add('review-shake'); setTimeout(()=>textarea.classList.remove('review-shake'),500); return; }

                // Page-rip/curl send animation on the textarea wrap
                wrap.classList.add('page-rip-out');

                setTimeout(() => {
                    // Save review
                    const reviews = getAppReviews(appId);
                    reviews.unshift({ text, timestamp: Date.now(), thumbsUp: 0, thumbsDown: 0, myVote: null });
                    saveAppReviews(appId, reviews);

                    // Refresh the reviews list in the open modal
                    renderReviewsList(appId);

                    // Switch the modal tab to Reviews
        ...
        const NEWS_ARTICLES=[
            {
                id:'v24',icon:'fa-sparkles',title:'iOS App Archive v2.4',date:'May 10, 2026',badge:'V2.4',

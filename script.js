        ...
        let viewedApps = []; // Track apps the user has viewed
        const MAX_VIEWED_APPS = 50; // Keep track of last 50 viewed apps
        ...
        function loadViewedApps() {
            try {
                ...
                console.error('Error loading viewed apps:', e);
                viewedApps = [];
            }
        }

        // Save viewed apps to localStorage
        function saveViewedApps() {
            try {
                localStorage.setItem('viewedApps', JSON.stringify(viewedApps));
            } catch (e) {
                console.error('Error saving viewed apps:', e);
            }
        }

        // Track when a user views an app
        function trackAppView(appId) {
            if (!appId) return;
            
            // Remove if already exists (to move to front)
            viewedApps = viewedApps.filter(id => id !== appId);
            
            // Add to beginning
            viewedApps.unshift(appId);
            
            // Keep only the last MAX_VIEWED_APPS
        ...
        // Generate Genius recommendations based on viewed apps
        function generateGeniusRecommendations() {
            if (!appsLoaded || apps.length === 0 || viewedApps.length === 0) {
                return [];
            }

            // Get the viewed app objects
            const viewedAppObjects = viewedApps
                .map(id => apps.find(app => app.id === id))
                .filter(Boolean)
                .slice(0, 10); // Use last 10 viewed apps for recommendations

            if (viewedAppObjects.length === 0) {
                // If no viewed apps, return random popular apps
                return getDailyRandomApps(apps, 6);
            }

            // Collect categories and developers from viewed apps
            const categoryScores = {};
            const developerScores = {};
            
            viewedAppObjects.forEach((app, index) => {
                const weight = 1 / (index + 1); // More recent views have higher weight
                ...
                // Score developers
                if (app.developer) {
                    developerScores[app.developer] = (developerScores[app.developer] || 0) + weight;
                }
            });

            // Score all apps based on similarity
            const appScores = apps
                .filter(app => !viewedApps.includes(app.id)) // Exclude already viewed apps
                .map(app => {
                    let score = 0;
                    let reason = '';
                    ...
                    if (Array.isArray(app.categories)) {
                        app.categories.forEach(category => {
                            if (categoryScores[category]) {
                                ...
                                if (!reason) reason = `Similar to apps in ${category}`;
                            }
                        });
                    }
                    
                    // Score based on matching developer
                    if (app.developer && developerScores[app.developer]) {
                        score += developerScores[app.developer] * 3;
                        reason = `From ${app.developer}`;
                    }
                    
                    // Add small random factor for diversity
                    score += Math.random() * 0.5;
                    
                    if (!reason) reason = 'Based on your browsing';
            ...
            return appScores.slice(0, 6).map(item => ({
                ...item.app,
                recommendationReason: item.reason
            }));
        }

        // Render Genius recommendations
        function renderGeniusRecommendations() {
            const geniusCardsGrid = document.getElementById('geniusCardsGrid');
            if (!geniusCardsGrid) return;

            geniusCardsGrid.innerHTML = '';

            const recommendations = generateGeniusRecommendations();

            if (recommendations.length === 0) {
                geniusCardsGrid.innerHTML = `
                    <div style="text-align: center; padding: 40px; color: var(--text-medium); grid-column: 1 / -1;">
                        <i class="fas fa-lightbulb" style="font-size: 48px; margin-bottom: 20px; color: var(--genius-color);"></i>
                        <h3>Start Exploring!</h3>
                        <p>View some apps to get personalized recommendations</p>
                    </div>
                `;
                return;
            }
            ...
            recommendations.forEach(app => {
                ...
                card.innerHTML = `
                    <div class="genius-card-content">
                        <div class="genius-card-icon">
                            ${app.icon ? 
                        ...
                        <div class="genius-card-info">
                            <div class="genius-card-title">${escapeHtml(app.title)}</div>
                            <div class="genius-card-developer">${escapeHtml(app.developer)}</div>
                            <div class="genius-card-reason">${escapeHtml(app.recommendationReason)}</div>
                        </div>
                    </div>
                    <button class="genius-card-button" data-app-id="${app.id}">View Details</button>
                `;
                
                geniusCardsGrid.appendChild(card);
            });

            // Add click event listeners to buttons
            geniusCardsGrid.querySelectorAll('.genius-card-button').forEach(button => {
        ...
        // Initialize viewed apps tracking
        loadViewedApps();
        ...
        async function fetchAppsData() {
            try {
                ...
                // Try to get apps from Archive.org sync
                if (window.archiveSync) {
                    console.log('📡 Attempting Archive.org sync...');
                    const syncedApps = await window.archiveSync.getApps();
                    
                    if (syncedApps && syncedApps.length > 0) {
                        console.log(`✅ Loaded ${syncedApps.length} apps from Archive.org`);
                        return syncedApps;
                    }
                }
                
                // Fallback to static apps.json
                console.log('📄 Falling back to static apps.json...');
                ...
                console.log(`✅ Loaded ${data.length} apps from apps.json`);
                return data;
                
            } catch (error) {
                console.error('❌ Error loading apps:', error);
                
                // Last resort: try just the static file
                try {
                    const response = await fetch('apps.json');
                    const data = await response.json();
        ...
        function openModal(appId) {
            ...
            // Track this app view for Genius recommendations
            trackAppView(appId);
        ...
        tabs.forEach(tab => {
            tab.addEventListener('click', function() {
                ...
                if (tabName === 'featured') {
                    ...
                    renderGeniusRecommendations();
                } else {
                    // For updates and other tabs
                    carouselContainer.style.display = 'none';
                    searchContainer.style.display = 'none';
                    searchResults.classList.remove('active');

        ...
        function createModal(app) {
            ...
            const catTags = app.categories.map(c=>`<span class="modal-category-tag">${c}</span>`).join('');
            ...
            modal.innerHTML=`
                ...
                <div class="version-sheet-overlay">
                    <div class="version-sheet">
                        ...
                        <button class="action-btn version-sheet-cancel" style="width:100%;margin-top:12px;">Cancel</button>
                    </div>
                </div>`;
        ...
        function renderAppsForCategory(cat){
            const cc=tabContents.categories; cc.innerHTML='';
            const back=document.createElement('button'); back.className='back-to-categories-btn';
            back.innerHTML='<i class="fa-solid fa-arrow-left"></i><span>Categories</span>';
            back.addEventListener('click',()=>{ setUrlParam('category',''); renderCategoryList(); });
            cc.appendChild(back);
            const title=document.createElement('h3'); title.textContent=`"${cat}"`; title.style.cssText='text-align:center;margin:12px 0 16px;color:#1a1a1a;font-size:18px;font-weight:700;';
            cc.appendChild(title);
            const filtered=apps.filter(a=>Array.isArray(a.categories)&&a.categories.includes(cat));
            const sorted=[...filtered].sort((a,b)=>a.title.localeCompare(b.title));
            if(!sorted.length){ cc.innerHTML+='<p style="text-align:center;color:#999;padding:20px;">No apps in this category.</p>'; return; }
            const grid=document.createElement('div'); grid.className='category-apps-grid category-fade-in';
            sorted.forEach(app=>{

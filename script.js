        ...
        function createModal(app) {
            ...
            modal.querySelector('.modal-get-btn').addEventListener('click',()=>{
                modal.querySelector('.version-sheet-overlay').classList.add('active');
                // Wire tap animation on download buttons once sheet is visible
                setTimeout(()=>{
                    modal.querySelectorAll('.download-button').forEach(btn=>{
                        if(btn._tapWired) return; btn._tapWired=true;
                        btn.addEventListener('click',function(){
                            this.classList.remove('tapped'); void this.offsetWidth;
                            this.classList.add('tapped');
                            setTimeout(()=>this.classList.remove('tapped'),400);
                        });
                    });
                },300);
            });
        ...
        const NEWS_ARTICLES=[
            {
                id:'v311',icon:'fa-gem',title:'iOS App Archive V3.1.1',date:'May 19, 2026',badge:'V3.1.1',
                preview:'Hyper-realistic 2010–2012 skeuomorphic overhaul — brushed aluminium chassis, deep-well toggles, mechanical button physics, polycarbonate tab bar, and spring-back animations.',
                body:`<h3>💎 V3.1.1 — Skeuomorphic Masterwork</h3>
<h3>🔩 Brushed Aluminium Chassis</h3><ul>
<li>Canvas-procedural brushed metal texture rendered across the page background — thousands of micro-fine horizontal noise lines overlaid with a soft radial studio-light gradient.</li>
<li>Sharp milled perimeter bevel: 1px white top edge, 1px dark bottom edge.</li>
</ul>
<h3>🖤 Polycarbonate Tab Bar</h3><ul>
<li>Multi-stop gradient simulating a curved glass capsule: dark charcoal → specular streak → obsidian → soft grey lip.</li>
<li>Each tab is an individually molded semi-glossy button with stacked box-shadows for physical separation and micro-reflections.</li>
<li>Specular streak shifts position on hover, simulating raking light across curved plastic.</li>
<li>Active tab pressed deep into the bar with concave inset shadow — looks carved into the polycarbonate.</li>
</ul>
<h3>⚙️ Mechanical Button Physics</h3><ul>
<li>Click depression: punchy cubic-bezier spring compression, shadows collapse 70%, button sinks 1px.</li>
<li>Spring-back release: slight overshoot bounce (0.5px) then snaps to rest.</li>
<li>All Get/action buttons use chiseled metallic texture with deep stamped/engraved text.</li>
</ul>
<h3>🔘 Deep-Well Toggle Switches — Fixed</h3><ul>
<li>Toggle track is a deep recessed well carved into the chassis — heavy inset shadows simulate physical depth.</li>
<li>ON/OFF labels stamped into the track floor, partially obscured by the sliding knob's shadow.</li>
<li>Knob slides with friction, hits the far wall with kinetic momentum and a micro-rebound bounce.</li>
<li>Fixed the duplicate-circle bug from V3.1.0.</li>
</ul>
<p>Thanks for using iOS App Archive!</p>`
            },
            {
                id:'v310',icon:'fa-star',title:'iOS App Archive V3.1.0',date:'May 18, 2026',badge:'V3.1.0',
                preview:'Settings overhaul with Customisation + Accessibility sections, Get button alignment fix, header centering, badge glow fix, download animation polish.',
                body:`<h3>⭐ V3.1.0 — Settings & Polish</h3>
<h3>⚙️ Settings Overhaul</h3><ul>
<li><strong>Customisation section</strong> — Accent colour, star ratings toggle, device icons toggle.</li>
<li><strong>Accessibility section</strong> (figure icon) — Reduce Motion, Large Text, High Contrast.</li>
<li>ON/OFF labels now sit <em>inside</em> the toggle track itself.</li>
<li>Top bar locked sticky. Version number centred at the bottom, larger.</li>
<li>"iOS App Archive" description line removed. "Clear Favourites" removed.</li>
</ul>
<h3>🔧 Fixes</h3><ul>
<li>Get button on category cards and featured cards is now pinned to the bottom — no more jumping for short-description apps.</li>
<li>iOS App Archive logo + text centred in header even with the settings button present.</li>
<li>Genius badge glow-pulse animation working correctly.</li>
<li>Download modal popup is smoother and slower. Download button has a tap animation.</li>
<li>Featured cards: removed drop shadow, added subtle 3D depth tilt instead.</li>
</ul>
<p>Thanks for using iOS App Archive!</p>`
            },
            {
...
<li><strong>Glossy Skeuomorphic UI</strong> — dark brushed-metal tab bar, iOS 6 aesthetic.</li>
        ...
        (function initDepthPush() {
            ...
            // ── Brushed aluminium canvas texture ──────────────────────────────
        function initBrushedMetal() {
            const canvas = document.getElementById('brushedMetalCanvas');
            if (!canvas) return;
            ...
            function render() {
                ...
                // Horizontal micro-noise lines
                for (let y = 0; y < h; y++) {
                    const bright = 118 + Math.random() * 28; // 118-146 grey
                    ctx.fillStyle = `rgb(${bright},${bright},${bright})`;
                    ctx.fillRect(0, y, w, 1);
                }
                // Radial studio-light overlay
                const rg = ctx.createRadialGradient(w*0.5, h*0.28, 0, w*0.5, h*0.28, Math.max(w,h)*0.75);
                rg.addColorStop(0,   'rgba(255,255,255,0.34)');
                rg.addColorStop(0.45,'rgba(255,255,255,0.10)');
        ...
        function applySettings() {
            ...
            document.body.classList.toggle('hide-ratings',    s.showRatings===false);
            document.body.classList.toggle('hide-device-icons', s.showDeviceIcons===false);
        ...
        function openSettings() {
            ...
            function tog(id, checked) {
                return `<label class="toggle-switch">
                    ...
                    <span class="toggle-track"><span class="toggle-knob"></span><span class="toggle-on-label">ON</span><span class="toggle-off-label">OFF</span></span>
                </label>`;
            }
            panel.innerHTML = `
                <div class="settings-sheet">
                    ...
                    <div class="settings-body">
                        ...
                        <div class="settings-section-label"><i class="fas fa-paintbrush"></i> Customisation</div>
                        ...
                        <div class="settings-row settings-toggle-row">
                            ...
                            ${tog('toggleRatings', s.showRatings!==false)}
                        </div>
                        <div class="settings-row settings-toggle-row">
                            <span class="settings-row-label"><i class="fas fa-mobile-screen-button"></i> Show Device Icons</span>
                            ${tog('toggleDeviceIcons', s.showDeviceIcons!==false)}
                        </div>

                        <div class="settings-section-label" style="margin-top:18px"><i class="fas fa-universal-access"></i> Accessibility</div>
                        <div class="settings-row settings-toggle-row">
                            <span class="settings-row-label"><i class="fas fa-wind"></i> Reduce Motion</span>
                            ${tog('toggleMotion', !!s.reducedMotion)}
                        </div>
                        <div class="settings-row settings-toggle-row">
                            <span class="settings-row-label"><i class="fas fa-text-height"></i> Large Text</span>
                            ${tog('toggleLargeText', !!s.largeText)}
                        </div>
                        <div class="settings-row settings-toggle-row">
                            <span class="settings-row-label"><i class="fas fa-circle-half-stroke"></i> High Contrast</span>
                            ${tog('toggleHighContrast', !!s.highContrast)}
                        </div>

                        <div class="settings-section-label" style="margin-top:18px"><i class="fas fa-database"></i> Data</div>
                        ...
                        <div class="settings-version-footer">V3.1.0</div>
                    </div>
                </div>`;
            ...
            panel.querySelectorAll('.accent-swatch').forEach(sw=>{
                sw.addEventListener('click',()=>{
                    panel.querySelectorAll('.accent-swatch').forEach(x=>x.classList.remove('active'));
            ...
            panel.querySelector('#toggleLargeText').addEventListener('change',function(){
                const ns=getSettings(); ns.largeText=this.checked; saveSettings(ns); applySettings();
            });
            panel.querySelector('#toggleHighContrast').addEventListener('change',function(){
                const ns=getSettings(); ns.highContrast=this.checked; saveSettings(ns); applySettings();
            });
            panel.querySelector('#clearRecentBtn').addEventListener('click',()=>{
                localStorage.removeItem('recentlyViewed'); renderRecentlyViewed();
                const b=panel.querySelector('#clearRecentBtn'); b.textContent='Done!';
                setTimeout(()=>b&&(b.textContent='Clear'),1500);
            });
            panel.querySelector('#clearGeniusBtn').addEventListener('click',()=>{
                localStorage.removeItem('viewedApps'); viewedApps=[];
                renderGeniusPage(); setTimeout(initGeniusPTR,120);
                const b=panel.querySelector('#clearGeniusBtn'); b.textContent='Done!';
        ...
        // ════════════════════════════════════════════════════════════════════
        // V3.1.1 — Mechanical Physics: Toggle kinetics + Button spring-back
        // ════════════════════════════════════════════════════════════════════

        // ── Mechanical toggle animation (kinetic friction + wall rebound) ──
        document.addEventListener('change', function(e) {
            const input = e.target;
            if (!input || input.type !== 'checkbox') return;
            const sw = input.closest('.toggle-switch');
            if (!sw) return;
            // Remove both anim classes, force reflow, add new one
            sw.classList.remove('animating-on', 'animating-off');
            void sw.offsetWidth; // reflow
            sw.classList.add(input.checked ? 'animating-on' : 'animating-off');
            // Clean up after animation
            setTimeout(() => sw.classList.remove('animating-on', 'animating-off'), 380);
        }, true);

        // ── Button spring-back: add no-spring on load, remove on first click ──
        document.addEventListener('DOMContentLoaded', () => {
            // Prevent spring animation on every new button added dynamically
            document.addEventListener('pointerdown', function(e) {
                const btn = e.target.closest('.action-btn, .get-btn, .large-card-button, .cat-get-btn');
                if (btn) btn.classList.remove('no-spring');
            }, true);
        });

        // ── Tab specular shift on click (polycarbonate light raking) ─────────
        document.querySelectorAll && document.addEventListener('DOMContentLoaded', () => {
            document.querySelectorAll('.tab').forEach(tab => {
                tab.addEventListener('pointerenter', function() {
                    this.style.backgroundPositionY = '-3px';
                });
                tab.addEventListener('pointerleave', function() {
                    this.style.backgroundPositionY = '0px';
                });
                tab.addEventListener('pointerdown', function() {
                    this.style.backgroundPositionY = '2px';
                });

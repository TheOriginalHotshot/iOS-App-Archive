...
class ArchiveOrgAutoSync {
        ...
        try {
            ...
            for (const item of items) {
                try {
                    ...
                    console.error(`❌ Error processing item ${item.identifier}:`, error);
            ...
            console.log(`✅ Sync complete! Processed ${this.processedApps.length} apps`);
    ...
    async fetchAllUserItems() {
        ...
        while (hasMore) {
            try {
                // Use Archive.org Scraping API - publicly accessible
                const url = cursor 
                    ? `https://archive.org/services/search/v1/scrape?q=uploader%3A%40${this.username}&fields=identifier,title,description,item_size&count=100&cursor=${cursor}`
                    : `https://archive.org/services/search/v1/scrape?q=uploader%3A%40${this.username}&fields=identifier,title,description,item_size&count=100`;
                ...
                if (data.items && data.items.length > 0) {
                    ...
                    cursor = data.cursor || '';
                    hasMore = data.items.length === 100; // If we got 100, there might be more
                } else {
                    hasMore = false;
                }
